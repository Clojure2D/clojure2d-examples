...
(def window (show-window cnvs "L-System"))
  ...
  (let [ndepth (inc depth)]
    (if (<= ndepth maxdepth) 
      (let [nsentence (generator sentence)]
        (draw-sentence cnvs init-pos turtle nsentence ndepth)
...
(defmethod key-pressed [(:window-name window) \space] [_ _]
  (let [r (rulesets (rand-nth (keys rulesets)))
    ...
    (println b)
    (draw-sentence cnvs (:init-pos b) (:turtle b) (:sentence b) (:depth b))
    b))

(let [b (set-state! window (:penrose-tiling rulesets))]
  (println b)
  (draw-sentence cnvs (:init-pos b) (:turtle b) (:sentence b) 0))
