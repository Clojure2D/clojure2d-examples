...
(time (let [filter (effects-filter (s/compose-effects effect1 effect2))
            res (->> p
...
(time (let [filter (effects-filter (s/effect :divider {:denominator 2}))
            res (->> p
...
;; fm filter
(time (let [eff (effects-filter (s/effect :fm {:quant 10 :omega (* m/TWO_PI 0.00225857) :phase 0.00822}) (width p))
...
(time (let [eff (effects-filter (s/effect :echo))
            res (p/filter-channels eff p)]
        (p/set-canvas-pixels! cnvs res)))

;; Good way
(time (let [effect-r (effects-filter (s/effect :echo))
            effect-g (effects-filter (s/effect :echo))
            effect-b (effects-filter (s/effect :echo))
            res (p/filter-channels effect-r effect-g effect-b nil p)]
        (p/set-canvas-pixels! cnvs res)))

;; vcf303 normalize (or equalize) after filtering
(time (let [eff (effects-filter (s/effect :vcf303 {:trigger true}) (* 50 ^long (width p)))
...
(s/save-signal (pixels->signal p {:planar? false
                                  :coding :alaw
                                  :signed? true
                                  :channels [2 0 1]
