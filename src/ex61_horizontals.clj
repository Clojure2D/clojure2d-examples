...
(def res (binding [f/*skip-random-fields* (r/brand 0.9)]
         (let [b1 (r/drand -12.0 -3.0)
               b2 (- b1)
               field (f/random-field)
               #_#_pal (c/palette) 
               #_#_pal [:black (c/gray 100) :white (c/gray 127.5) :black (c/gray 155) :white]
               pal [:black (c/gray 100) (c/gray 155) :white]
               palcnt (dec (count pal))]
           (c2d/with-canvas [c (c2d/canvas 600 600)]
             (c2d/set-background c (c/gray 240))
             (doseq [y (range 600)
                     :let [yy (m/norm y 0.0 600.0 b1 b2)
                           step (-> (v/vec2 b1 yy) field v/mag m/sin m/sq inc (* 0.5))]]
               (loop [xx (double b1)]
                 (when (< xx b2)
                   (let [fv (field (v/vec2 xx yy))
                         nxx (+ xx step)
                         x (m/norm xx b1 b2 0.0 600.0)
                         nx (m/norm nxx b1 b2 0.0 600.0)
                         h (m/sin (v/heading fv)) ;; v/mag
                         pv (m/round (m/norm h -1.0 1.0 0.0 palcnt))]
                     (c2d/set-color c (pal pv))
                     (c2d/line c x y nx y)
                     (recur nxx)))))
             (let [img (o/render-noise
                        (c2d/to-image c) no)]
               (u/show-image img)
               img)))))
