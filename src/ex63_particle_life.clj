...
(defrecord ParticleType [^long id ^double rmax color ^double friction])
...
(defn random-setup
  ...
  (let [n2 (m/* n n)
    ...
    (map->Setup {:n n :alpha alpha :beta beta :types types :fmult (r/drand 1.0 50.0)})))
    ...
    (-> (reduce (fn [^Vec2 f ^Particle p]
                  (let [^ParticleType ptype (.ptype p)
                    ...
                    (if (and (m/pos? d) (m/< d rmax))
                      (v/add f (v/mult (v/div r d) (F (m// d rmax)
...
(defn move [setup particles ^double dt]
  (pmap (fn [^Particle p]
          (let [^ParticleType ptype (.ptype p)
                ...
                [np nv] (fix-vel np nv)]
            (Particle. np nv ptype))) particles))
...
(def window (c2d/show-window {:canvas (c2d/black-canvas csize csize :highest)
                            :draw-fn draw
                            :background :black
                            :draw-state (let [setup (random-setup (r/irand 2 10))]
                                          (println setup)
