(ns rt4.the-next-week.ch03.material
  ...
  (scatter [_ ray-in rec]
    (let [scatter-direction (v/add (:normal rec) (common/random-unit-vector))]
      (if (v/is-near-zero? scatter-direction)
        (->MaterialData albedo (ray/ray (:p rec) (:normal rec) (:time ray-in)))
        (->MaterialData albedo (ray/ray (:p rec) scatter-direction (:time ray-in)))))))
